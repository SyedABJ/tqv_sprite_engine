...
- Each sprite has a **flip control bit** to horizontally mirror the sprite
...
| 0x01    | spr0_ctrl       | R/W    | **[0]** flip (0=normal, 1=horizontal mirror)                                |
| 0x02    | spr1_ctrl       | R/W    | **[0]** flip (0=normal, 1=horizontal mirror) 
...
2. Write sprite coordinates into `spr0_xy` and `spr1_xy`.  
3. Write the 64-bit bitmaps into the corresponding `spr*_bmp` registers in 16-bit chunks.  
4. (Optional) Write `spr*_ctrl[0] = 1` to enable horizontal flip.  
5. Enable streaming by setting `control_reg[0] = 1`.  
6. Connect a VGA/XGA monitor and observe output.  
7. Enable interrupts by setting `control_reg[1] = 1`.  
